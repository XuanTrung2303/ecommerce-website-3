                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.products.index') }}" class="btn btn-success shadow-sm float-right"> <i
                                    class="fa fa-arrow-left"></i> Return</a>
                            <a href="{{ route('admin.products.product_images.index', $product) }}"
                                class="btn btn-success shadow-sm mr-2 float-right"> <i class="fa fa-image"></i> Upload
                                Product Images
                            </a>
                        </div>
                        <!-- /.card-header -->
                            ...
                            <form method="post" action="{{ route('admin.products.update', $product) }}">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" name="type" id="type">
                                            @foreach ($types as $value => $type)
                                                <option {{ old('type', $product->type) == $value ? 'selected' : null }}
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <select class="form-control select-multiple" multiple="multiple"
                                            ...
                                            @foreach ($categories as $category)
                                                <option
                                                    {{ in_array($category->id, $product->categories->pluck('id')->toArray()) ? 'selected' : null }}
                                ...
                                @if (!empty($configurable_attributes) && empty($product))
                                    ...
                                    @foreach ($configurable_attributes as $configurable_attribute)
                                        <div class="form-group row border-bottom pb-4">
                                            <label for="{{ $configurable_attribute->code }}"
                                                class="col-sm-2 col-form-label">{{ $configurable_attribute->code }}</label>
                                            <div class="col-sm-10">
                                                <!-- sampai sini -->
                                                <select class="form-control select-multiple" multiple="multiple"
                                                    name="{{ $configurable_attribute->code }}[]"
                                                    id="{{ $configurable_attribute->code }}">
                                                    @foreach ($configurable_attribute->attribute_options as $attribute_option)
                                                        <option value="{{ $attribute_option->id }}">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" name="status" id="status">
                                                @foreach ($statuses as $value => $status)
                                                    <option
                                                        {{ old('status', $product->status) == $value ? 'selected' : null }}
